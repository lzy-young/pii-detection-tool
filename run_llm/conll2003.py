...
- You ONLY can use the categories mentioned above to classify the extracted information.
===============================================================================================
...
- Each input is independent, <B> indicates the start of the text and <E> indicates the end of the text
- Generate <END> after processing the text to indicate the end of your output
...
OUTPUT:
{{"value": "John Smith", "label": "PER", "start": 0, "end": 10}}
{{"value": "New York", "label": "LOC", "start": 74, "end": 82}}
{{"value": "OpenAI", "label": "ORG", "start": 97, "end": 103}}
{{"value": "Japan", "label": "LOC", "start": 118, "end": 123}}
<END>
===============================================================================================
INPUT: 
<B> {text} <E>
...
if __name__ == "__main__":
    ...
    gc.collect()
    args=parse_args()
    llm=LLM(model=args.model_path,tensor_parallel_size=args.tensor_parallel_size,seed=args.seed,gpu_memory_utilization=args.gpu_memory_utilization,trust_remote_code=True)
    sampling_params = SamplingParams(
    temperature=0,
    max_tokens=1024,
    stop=["\n\n\n","###","```","python","<END>","<E>","<B>","Example","</E>","</B>","</think>"],
    n=1
    )
    ...
    if '--debug' in sys.argv:
        print("🔍 启动调试模式...")
        debug(data_path, llm, sampling_params, debug_count=20)
        exit(0)
