...
- You CAN'T output any explanation and your thinking process, ONLY the JSONL output as specified
- DO NOT generate the SAME entity multiple times, each value should only appear ONCE in the output
========================================================================================
CATEGORIES LIST:
[TIME]: Any time-related information such as timestamps, dates, durations (e.g. "10:30 AM", "7", "a quarter past 2", etc.)
[USERNAME]: Usernames or online handles associated with individuals. (e.g. "john_doe123", "alice1987","hunt.auther133") 
[GIVENNAME]: The first name of an individual. (e.g. "John", "Alice", "Michael")
[LASTNAME]: The last name of an individual. (e.g. "Doe", "Smith", "Johnson", "Williams",etc.)
[IDCARD]: Identification card numbers. (e.g. "A1234567", etc.)
[SOCIALNUMBER]: Social security numbers. (e.g. "123-45-6789", etc.)
[PASSPORT]: Passport numbers. (e.g. "P1234567", etc.)
[DRIVERLICENSE]: Driver's license numbers. (e.g. "D7654321", etc.)
[BOD]: Date of birth. (e.g. "January 1, 1990", "1990-01-01", etc.)
[IP]: IP addresses. (e.g. "192.168.1.1", etc.)
[SEX]: Gender information. (e.g. "Male", "Female", "Non-binary", etc.)
[EMAIL]: Email addresses. (e.g. "young@123.com", etc.)
[TEL]: Telephone numbers. (e.g. "+1-800-555-1234", etc.)
[CITY]: City names. (e.g. "New York", "Los Angeles", etc.)
[STREET]: Street names. (e.g. "123 Main St", etc.)
[STATE]: State names. (e.g. "California", "Texas", etc.)
[BUILDING]: Building names. (e.g. "Empire State Building", etc.)
[COUNTRY]: Country names. (e.g. "United States", "Canada", etc.)
[POSTCODE]: Postal codes. (e.g. "10001", "90210", etc.)
[TITLE]: Job titles. (e.g. "Dr.", "Prof.", "Mr.", "Ms.", etc.)
[DATE]: Dates. (e.g. "October 5th, 2023", "2023-10-05", etc.)
[PASS]: Passwords. (e.g. "%123*qwe", etc.)
[SECADDRESS]: Security addresses. (e.g. "Floor 4", "Apartment 12B", etc.)
[GEOCOORD]: Geographical coordinates. (e.g. "23.1234, 113.1234", etc.)
========================================================================================
EXAMPLES:
EXAMPLE1:
INPUT: 
...
EXAMPLE2:
INPUT: 
<B> "I have 2 apples. And they are worth 5 dollars. I bought them on October 21st, 2024." <E>
OUTPUT:
{{"value": "October 21st, 2024", "label": "DATE", "start": 70, "end": 90}}
<END>
...
EXAMPLE4:
INPUT: 
<B> "PASSWORD: %123*qwe, USERNAME: young.johnson123, DATE: October 5th, 2023, SEX: Male, IP: 192.168.1.1, TITLE: Dr., LASTNAME: Johnson." <E>
OUTPUT:
...
def calculate_metrics(results, labels):
    ...
    results_sets['DATE'].update(results_sets['BOD'])
    labels_sets['DATE'].update(labels_sets['BOD'])
    for entity_type in results_sets.keys():
        if entity_type == 'BOD':
            continue  # 跳过BOD类型的计算
...
def debug(data_path, model, sampling_params, debug_count=50):
    ...
    try:
        ...
        for i, item in enumerate(iter_data):
            ...
            try:
                # 处理文本（可能包含多行）
                texts = item['source_text'].strip().replace('\\n','\n').split('\n')
                texts= [text for text in texts if len(text.strip())>0 and not is_only_special_chars(text)]
...
if __name__ == "__main__":
    ...
    max_tokens=512,
    stop=["\n\n\n","###","```","python","<END>","<E>","<B>","Example","-->","<--","wrong","Okay"],
    n=1
    )
