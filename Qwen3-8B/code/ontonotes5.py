...
def build_resume_list(results_path,data_path):
    ...
    for file in os.listdir(data_path):
        ...
        if resume_cnt!=-1:
            tp=result_data.get('batches',{}).get(f'batch_{batch_cnt}',{}).get('tp',0)
            fp=result_data.get('batches',{}).get(f'batch_{batch_cnt}',{}).get('fp',0)
            fn=result_data.get('batches',{}).get(f'batch_{batch_cnt}',{}).get('fn',0)
            metrics=[tp,fp,fn]
