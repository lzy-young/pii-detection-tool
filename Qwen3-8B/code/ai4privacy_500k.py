...
def create_jsonl_prompt(text,locale='US'):
    """创建要求JSONL输出的提示词"""
    prompt = f"""TASK: ONLY Extract PII information and output in JSONL format. DO NOT output ANY other text. 

INSTRUCTIONS:
- output the PII entities found in the INPUT text in JSONL format FIRSTLY. 
- Extract EVERY entity mention that appears in the INPUT text. Do not miss any mention.
...
- If there is no entity, output {{"message": "No PII detected"}}
- Try to improve your precision according to the EXAMPLES.
- the REGION is {locale}. Take locale context into consideration when extracting entities. 
